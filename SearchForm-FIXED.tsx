...
export default function SearchForm({ onSearch, isSearching, setIsSearching }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      // First, get all churches from the database
      ...
      // Call the Edge Function with the search parameters
      const { data: matchData, error: matchError } = await supabase.functions.invoke('match-church', {
        body: {
          ...
          priorities,
          additionalInfo,
          churches, // Pass the churches to the function
        },
      });
      ...
      // Transform the data to match what SearchResults expects
      ...
      // Find the full church data for each match
      ...
      // Format the results with reasons from AI
      const results = {
        bestMatch: {
          ...bestMatchChurch,
